...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        filtered_df = pd.DataFrame(filtered_df.groupby(['Launch Site','class']).size().reset_index(name='count'))
...
def get_scatter_chart(entered_site, slider_number):
    spacex1_df=spacex_df[(spacex_df['Payload Mass (kg)'] > slider_number[0]) & (spacex_df['Payload Mass (kg)'] < slider_number[1])]
    if entered_site == 'ALL':
        fig = px.scatter(spacex1_df,  x='Payload Mass (kg)', y='class', color='Booster Version Category',
        title='Correlation betwen Payload and Success for All Sites')
        return fig
        # return the outcomes piechart for a selected site
    else:
        spacex1_df = spacex1_df[spacex1_df['Launch Site']==entered_site]
        fig = px.scatter(spacex1_df,  x='Payload Mass (kg)', y='class', color='Booster Version Category',
